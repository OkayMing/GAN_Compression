...
import librosa
import numpy as np
class Audio_VGGExtractor(nn.Module):
    def __init__(self,d):
        super(Audio_VGGExtractor, self).__init__()
...
class Audio_VGGLoss(nn.Module):
    def __init__(self, d=40,sampling_ratio = 16000, n_fft=512, n_mels = 128,path = None):
        super(Audio_VGGLoss, self).__init__()
        self.vgg = Audio_VGGExtractor(d=d).cuda()
